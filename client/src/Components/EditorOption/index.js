...
class EditorOption {
  ...
  constructor({ data, config, api, readOnly }) {
    ...
    this.newData = data;
    this.colId =
      this.api.ui.nodes.wrapper.parentElement.id?.split(" ")?.[1] +
      " " +
      "top-1";
    ...
    /* this code comented to fix the dropdown undefined issue */
    //  this.lastChild  = this.optionElement;
    //  this.editorData = this.lastChild.getElementsByClassName('cdx-block')[0]?.innerText

    /* the dropdown undefined issue fixed*/
    if (this.optionElement) {
      this.lastChild = this.optionElement;
      const cdxBlockElement =
        this.lastChild.getElementsByClassName("cdx-block")[0];
      this.editorData = cdxBlockElement ? cdxBlockElement.innerText : "";
      console.log(this.editorData, "this.editorData---2");
    } else {
      console.warn(`Element with ID ${this.colId} not found`);
      this.editorData = ""; // default value to avoid undefined errors
    }
    ...
    this._placeholder = config.placeholder
      ? config.placeholder
      : EditorOption.DEFAULT_PLACEHOLDER;
    this._data = {};
  ...
  drawView() {
    ...
    console.log(this.newData, "this.newData",this.editorData,"data, z", this.optionElement);
    // Set the selected value if data exists
    if (this.newData.text) {
      this.selectList.innerText = "";
      const opts = document.createElement("option");
    ...
    if (!this.readOnly) {
      // Add change event listener to update state and re-render
      this.selectList.addEventListener("change", () => {
        this.newData = { text: this.selectList.value };
        this.render();
      });
    }
  ...
  _rerender() {
    ...
    this.lastChild = this.optionElement;
    if (!this.lastChild) {
      console.error("Unable to find the expected DOM element.");
    }
    this.editorData =
      this.lastChild.getElementsByClassName("cdx-block")[0].innerText;
